...
func (m mockSQS) SendMessageBatchWithContext(ctx aws.Context, input *awssqs.SendMessageBatchInput, opts ...request.Option) (*awssqs.SendMessageBatchOutput, error) {
	m.Called(input)
	msgCount += len(input.Entries)
	return &awssqs.SendMessageBatchOutput{}, nil
}

func (m mockSQS) ReceiveMessageWithContext(ctx aws.Context, input *awssqs.ReceiveMessageInput, opts ...request.Option) (*awssqs.ReceiveMessageOutput, error) {
	args := m.Called(input)
	return args.Get(0).(*awssqs.ReceiveMessageOutput), nil
}

func (m mockSQS) DeleteMessageBatchWithContext(ctx aws.Context, input *awssqs.DeleteMessageBatchInput, opts ...request.Option) (*awssqs.DeleteMessageBatchOutput, error) {
	m.Called(input)
...
func TestSQSQueue_PutMessages_Success1(t *testing.T) {
	mockedQueue, assertStubs := newMockQueue(func(client *mockSQS) {
		client.On("SendMessageBatchWithContext", mock.Anything).Once()
...
func TestSQSQueue_PutMessages_Success2(t *testing.T) {
	msgCount = 0
	mockedQueue, assertStubs := newMockQueue(func(client *mockSQS) {
		client.On("SendMessageBatchWithContext", mock.Anything).Times(2)
		// FIXME - id of messages are generated uuid so we can't expect the value
	})
...
func TestSQSQueue_PutMessagesWithContext_Success1(t *testing.T) {
	msgCount = 0
	mockedQueue, assertStubs := newMockQueue(func(client *mockSQS) {
		client.On("SendMessageBatchWithContext", mock.Anything).Once()
		// FIXME - id of messages are generated uuid so we can't expect the value
	...
	err := mockedQueue.PutMessagesWithContext(context.Background(), make([]string, 3))
...
// If the size of messages is larger than 10, the messages have been split by
// 10 and call API multiple times.
func TestSQSQueue_PutMessagesWithContext_Success2(t *testing.T) {
	msgCount = 0
	mockedQueue, assertStubs := newMockQueue(func(client *mockSQS) {
		client.On("SendMessageBatchWithContext", mock.Anything).Times(2)
		// FIXME - id of messages are generated uuid so we can't expect the value
	...
	err := mockedQueue.PutMessagesWithContext(context.Background(), make([]string, 15))
...
func TestSQSQueue_FetchMessages_Success1(t *testing.T) {
	mockQueue, assertStubs := newMockQueue(func(client *mockSQS) {
		...
		client.On("ReceiveMessageWithContext", expect1).Return(makeReceiveMessageOutput(3), nil).Once()
		...
		client.On("DeleteMessageBatchWithContext", expect2).Once()
	})
...
func TestSQSQueue_FetchMessages_Success2(t *testing.T) {
	mockQueue, assertStubs := newMockQueue(func(client *mockSQS) {
		...
		client.On("ReceiveMessageWithContext", expect).Return(makeReceiveMessageOutput(0), nil).Once()
	})
...
func TestSQSQueue_FetchMessagesWithContext_Success1(t *testing.T) {
	mockQueue, assertStubs := newMockQueue(func(client *mockSQS) {
		expect1 := &awssqs.ReceiveMessageInput{MaxNumberOfMessages: aws.Int64(10), QueueUrl: &queueURL, WaitTimeSeconds: aws.Int64(20)}
		client.On("ReceiveMessageWithContext", expect1).Return(makeReceiveMessageOutput(3), nil).Once()
	...
	messages, err := mockQueue.FetchMessagesWithContext(context.Background())
	assert.Nil(t, err)
	assert.Equal(t, 3, len(messages), "number of messages should be 3")
	assertStubs(t)
}

func TestSQSQueue_FetchMessagesWithContext_Success2(t *testing.T) {
	mockQueue, assertStubs := newMockQueue(func(client *mockSQS) {
		expect := &awssqs.ReceiveMessageInput{MaxNumberOfMessages: aws.Int64(10), QueueUrl: &queueURL, WaitTimeSeconds: aws.Int64(20)}
		client.On("ReceiveMessageWithContext", expect).Return(makeReceiveMessageOutput(0), nil).Once()
	})
